...
$isCompanyConfigAllowed = in_array($userRole, ['owner', 'admin', 'manager'], true);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        $ownerOnlyActions = [
            ...
            'update_license_pricing',
            'update_security_notifications',
            'update_about',
            'update_settings',
        ...
        if (in_array($action, $ownerOnlyActions, true) && !$isOwner) {
            $error = 'Only the owner can perform that action.';
        } elseif ($action === 'update_app_name') {
            $currentUser = getCurrentUser();
            if ($currentUser['role'] !== 'owner') {
                ...
                saveSetting($conn, 'show_logo_in_sidebar', isset($_POST['show_logo_in_sidebar']) ? '1' : '0');
            ...
            if (!$isCompanyConfigAllowed) {
                $error = 'You do not have permission to update tax settings.';
            } else {
                $tax_rate = floatval($_POST['tax_rate'] ?? TAX_RATE);
                if ($currentUser['role'] === 'owner') {
                    saveSetting($conn, 'tax_rate', $tax_rate);
                } else {
                    $companyId = intval($currentUser['company_id'] ?? 0);
                    if ($companyId > 0) {
                        saveSetting($conn, 'tax_rate_company_' . $companyId, $tax_rate);
                    }
                }
                $success = 'Tax/VAT settings saved successfully!';
            }
        ...
        } elseif ($action === 'update_license_pricing') {
            saveSetting($conn, 'license_currency', sanitize($_POST['license_currency'] ?? '$'));
            saveSetting($conn, 'license_starter_price', sanitize($_POST['license_starter_price'] ?? 'Free'));
            saveSetting($conn, 'license_professional_price', floatval($_POST['license_professional_price'] ?? 99));
            saveSetting($conn, 'license_enterprise_price', sanitize($_POST['license_enterprise_price'] ?? 'Custom'));
            $success = 'License pricing saved successfully!';
        ...
        <div class="card-body">
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="company_name" class="form-control form-control-lg" value="<?= htmlspecialchars($settings['company_name'] ?? 'Vendrixpos', ENT_QUOTES) ?>" placeholder="Enter app name">
                ...
                <div class="mb-3 form-check">
                    <input type="checkbox" class="form-check-input" id="show_logo_in_sidebar" name="show_logo_in_sidebar" value="1" <?= ($settings['show_logo_in_sidebar'] ?? '0') === '1' ? 'checked' : '' ?>>
                    <label class="form-check-label" for="show_logo_in_sidebar">Show logo in sidebar instead of app name</label>
                </div>
    ...
    <div class="card-header">
        <ul class="nav nav-tabs card-header-tabs" id="settingsTabs" role="tablist">
            <?php if ($isOwner): ?>
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="tab-license-pricing" data-bs-toggle="tab" data-bs-target="#license-pricing" type="button" role="tab">License Pricing</button>
                </li>
                <li class="nav-item" role="presentation">
            ...
            <?php endif; ?>
            <?php if ($isCompanyConfigAllowed): ?>
                <li class="nav-item" role="presentation">
                    <button class="nav-link <?= $isOwner ? '' : 'active' ?>" id="tab-tax-settings" data-bs-toggle="tab" data-bs-target="#tax-settings" type="button" role="tab">Tax & VAT</button>
                </li>
            <?php endif; ?>
            <li class="nav-item" role="presentation">
                <button class="nav-link <?php echo !$isOwner && !$isCompanyConfigAllowed ? 'active' : ''; ?>" id="tab-close-account" data-bs-toggle="tab" data-bs-target="#close-account" type="button" role="tab">Close Account</button>
            </li>
        </ul>
    </div>
    <div class="card-body">
        <div class="tab-content" id="settingsTabsContent">
            <?php if ($isOwner): ?>
                <div class="tab-pane fade <?php echo $isOwner ? 'show active' : ''; ?>" id="login-settings" role="tabpanel">
                    <form method="POST">
                        <input type="hidden" name="csrf_token" value="<?= generateCSRFToken() ?>">
            ...
            <?php if ($isOwner): ?>
                ...
                <div class="tab-pane fade" id="license-pricing" role="tabpanel">
                    <form method="POST">
                        <input type="hidden" name="csrf_token" value="<?= generateCSRFToken() ?>">
                        <input type="hidden" name="action" value="update_license_pricing">
                        <div class="row">
                            <div class="col-md-3">
                                <div class="mb-3">
                                    <label class="form-label"><strong>Currency Symbol</strong></label>
                                    <input type="text" name="license_currency" class="form-control" value="<?= htmlspecialchars($settings['license_currency'] ?? '$', ENT_QUOTES) ?>" maxlength="3">
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="mb-3">
                                    <label class="form-label"><strong>Starter Plan Label</strong></label>
                                    <input type="text" name="license_starter_price" class="form-control" value="<?= htmlspecialchars($settings['license_starter_price'] ?? 'Free', ENT_QUOTES) ?>">
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="mb-3">
                                    <label class="form-label"><strong>Professional Plan Price</strong></label>
                                    <input type="number" step="0.01" min="0" name="license_professional_price" class="form-control" value="<?= htmlspecialchars($settings['license_professional_price'] ?? '99', ENT_QUOTES) ?>">
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="mb-3">
                                    <label class="form-label"><strong>Enterprise Label</strong></label>
                                    <input type="text" name="license_enterprise_price" class="form-control" value="<?= htmlspecialchars($settings['license_enterprise_price'] ?? 'Custom', ENT_QUOTES) ?>">
                                </div>
                            </div>
                        </div>
                        <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Save License Pricing</button>
                    </form>
                </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="company_phone" class="form-control" value="<?= htmlspecialchars($settings['company_phone'] ?? '08080500766', ENT_QUOTES) ?>">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label class="form-label">Company Email</label>
                                    <input type="email" name="company_email" class="form-control" value="<?= htmlspecialchars($settings['company_email'] ?? 'info@vendrixpos.com', ENT_QUOTES) ?>">
                                </div>
                            </div>
                        </div>
            ...
            <?php if ($isCompanyConfigAllowed): ?>
                <div class="tab-pane fade <?= $isOwner ? '' : 'show active' ?>" id="tax-settings" role="tabpanel">
                    <form method="POST">
                        <input type="hidden" name="csrf_token" value="<?= generateCSRFToken() ?>">
                        <input type="hidden" name="action" value="update_tax_settings">
                        <div class="mb-3">
                            <label class="form-label"><strong>Tax / VAT Rate (%)</strong></label>
                            <input type="number" step="0.1" min="0" name="tax_rate" class="form-control" value="<?= htmlspecialchars($isOwner ? ($settings['tax_rate'] ?? '7.5') : ($settings['tax_rate_company_' . ($currentUser['company_id'] ?? 0)] ?? $settings['tax_rate'] ?? '7.5'), ENT_QUOTES) ?>" placeholder="Enter tax rate">
                            <small class="text-muted"><?= $isOwner ? 'Global tax rate for the app.' : 'Company tax/VAT rate for your company.' ?></small>
                        </div>
                        <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Save Tax/VAT Settings</button>
                    </form>
                </div>
            <?php endif; ?>

            <div class="tab-pane fade <?php echo !$isOwner && !$isCompanyConfigAllowed ? 'show active' : ''; ?>" id="close-account" role="tabpanel">
                <form method="POST">
                    <input type="hidden" name="csrf_token" value="<?= generateCSRFToken() ?>">
